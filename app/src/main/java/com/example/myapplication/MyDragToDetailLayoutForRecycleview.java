...
public class MyDragToDetailLayoutForRecycleview extends ViewGroup {
    ViewDragHelper viewDragHelper;

    private float startY;

    public MyDragToDetailLayoutForRecycleview(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        viewDragHelper = ViewDragHelper.create(this, new ViewDragHelper.Callback() {
            ...
            public void onViewPositionChanged(@NonNull View changedView, int left, int top, int dx, int dy) {
                ...
                ViewCompat.postInvalidateOnAnimation(MyDragToDetailLayoutForRecycleview.this);
            }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (ev.getActionMasked() == MotionEvent.ACTION_DOWN) {
            ...
            if (getChildAt(1).canScrollVertically(-1)) {//处理冲突
                isConflict = true;
            }
        }

        Constant.LOG("confit: "+isConflict);
        if (!isConflict) {
